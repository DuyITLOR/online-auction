  ...
  export const getAllProduct = async ({
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/product?${params}`
      );
      const data = await res.json();
  ...
  export const createProduct = async ({
    ...
    formData.append("autoExtendMinutes", product.extendMinutes.toString());
    ...
    if (!res.ok) {
      const message =
        data.message || data.error || "Failed in create product api";
  ...
  export const updateProduct = async ({
    ...
    try {
      ...
      if (!res.ok) {
        const message =
          data.message || data.error || "Failed to fetch in update product";
        throw new Error(message);
      }
